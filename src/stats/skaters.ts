...
import { StatsApiResponse, StatsQueryParams, StatsSkater, Milestone } from '../types/stats-api.js';

export class SkatersStatsEndpoints {
  ...
  async getByReport(reportType: string, query?: StatsQueryParams): Promise<StatsApiResponse<StatsSkater>> {
    return this.http.get<StatsApiResponse<StatsSkater>>(`/skater/${reportType}`, {
      params: this.buildParams(query),
    });
  }

  async getLeaders(reportType: string, query?: StatsQueryParams): Promise<StatsApiResponse<StatsSkater>> {
    return this.http.get<StatsApiResponse<StatsSkater>>(`/leaders/skaters/${reportType}`, {
      params: this.buildParams(query),
    });
  }

  async getMilestones(query?: StatsQueryParams): Promise<StatsApiResponse<Milestone>> {
    return this.http.get<StatsApiResponse<Milestone>>('/milestones/skaters', {
      params: this.buildParams(query),
    });
  }
}
